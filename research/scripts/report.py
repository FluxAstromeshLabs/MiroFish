...
def load_ohlcv(ohlcv_path: Path) -> pd.DataFrame:
    """Load OHLCV from a CSV with columns T,O,H,L,C,V (T in milliseconds)."""
...
def main():
    ...
    parser.add_argument("--ohlcv", default="research/data/ohlcv_1h.csv",
                        help="Path to OHLCV CSV file (columns: T,O,H,L,C,V; T in ms)")
    ...
    ohlcv = load_ohlcv(Path(args.ohlcv))
