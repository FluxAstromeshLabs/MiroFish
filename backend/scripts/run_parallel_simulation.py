"""
Preset OASIS dual-platform parallel simulation script.
Runs Twitter and Reddit simulations together from the same config file.

Features:
- Parallel simulation across both Twitter and Reddit
- Keeps the environment alive after the simulation finishes and enters command-wait mode
- Supports Interview commands over IPC
- Supports both single-agent and batch interviews
- Supports remote environment shutdown
...
# ============================================================
# Fix Windows encoding behavior by setting UTF-8 before any imports.
# This works around third-party OASIS code that reads files without specifying an encoding.
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard streams as UTF-8.
    ...
    # This ideally needs to happen at Python startup, so we also monkey-patch the built-in open().
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes third-party libraries such as OASIS that read files without specifying an encoding.
        """
        # Only set a default encoding for text mode when one is not provided.
...
# Load the project root `.env` file, including settings such as `LLM_API_KEY`.
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings; this script intentionally leaves max_tokens unset."""
...
# Add the filter during module import so it applies before camel runs.
...
def disable_oasis_logging():
    ...
    OASIS logs are too noisy because they record every agent observation and action; this script uses its own action logger.
    """
...
# Available Twitter actions. `INTERVIEW` is excluded and can only be triggered manually via `ManualAction`.
TWITTER_ACTIONS = [
...
# Available Reddit actions. `INTERVIEW` is excluded and can only be triggered manually via `ManualAction`.
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get the environment and agent graph for the specified platform.
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            A dict containing the result, or a dict containing `error`
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Optional target platform
                ...
                - `None`: interview on both platforms and return a combined result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Interviews that should run on both platforms.
        ...
        # Split `both_platforms_interviews` into the two platform queues.
    ...
    async def process_commands(self) -> bool:
        ...
        Returns:
            True if execution should continue, False if it should exit.
        """
...
# Non-core action types to filter out because they add little analytical value.
FILTERED_ACTIONS = {'refresh', 'sign_up'}

# Mapping of database action names to normalized action names.
ACTION_TYPE_MAP = {
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This lets `actions.jsonl` show real entity names instead of aliases like `Agent_0`.
    
    Args:
        config: Parsed contents of `simulation_config.json`
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Highest rowid read previously. Rowid is used instead of `created_at`
            because timestamp formats differ between platforms.
        agent_names: Mapping from `agent_id` to `agent_name`
        
    Returns:
        `(actions_list, new_last_rowid)`
        - `actions_list`: action records containing `agent_id`, `agent_name`, `action_type`, and contextualized `action_args`
    ...
    try:
        ...
        # Track processed rows using SQLite rowid.
        # This avoids cross-platform `created_at` format differences.
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Keep only the key fields in `action_args` without truncating content.
            simplified_args = {}
            ...
            # Enrich with context such as post content and user names.
...
def _enrich_action_context(
    ...
    """
    Enrich an action with contextual data such as post content and user names.
    ...
    try:
        ...
        # Repost: add original post content and author.
            ...
            if new_post_id:
                # For reposts, `original_post_id` points to the source post.
                cursor.execute("""
        ...
        # Quote post: add source content, source author, and quote comment.
            ...
            # Get the quoted comment content (`quote_content`).
            if new_post_id:
                cursor.execute("""
        ...
        # Context enrichment failure should not block the main flow.
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Fall back to the config file if `.env` does not provide a model name.
    if not llm_model:
...
def get_active_agents_for_round(
    ...
    """Decide which agents should be active this round based on time and config."""
...
async def run_twitter_simulation(
    ...
    # Load real agent names from config using `entity_name` instead of default `Agent_X`.
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed database row using rowid.
    ...
    # Cap the total number of rounds if requested.
    ...
    for round_num in range(total_rounds):
        # Stop early if an exit signal was received.
        ...
        # Record round start even if no agents are active.
        if action_logger:
        ...
        # Fetch and log the actions that actually ran.
    ...
    # Keep the environment alive for later interview commands.
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration when available, otherwise standard config.
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Shut down the environment immediately after the simulation instead of entering wait-for-commands mode'
    )
    
    args = parser.parse_args()
    
    # Create the shutdown event at the start of `main` so the program can react to exit signals.
    ...
    # Initialize logging, disable OASIS logs, and clear old files.
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use `wait_for` instead of `sleep` so shutdown signals are handled promptly.
                try:
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers so SIGTERM/SIGINT exits cleanly.
    
    This simulation can stay alive after completion while waiting for interview commands.
    When a termination signal arrives, it should:
    1. notify the asyncio loop to stop waiting
    2. allow normal resource cleanup
    3. exit afterward
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Signal the asyncio loop so it can exit and clean up resources.
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not call `sys.exit()` immediately; let asyncio exit cleanly first.
