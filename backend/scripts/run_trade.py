...
def _load_graph_cache(sidecar_path: str, sha256: str) -> Optional[Tuple[str, str]]:
...
def _save_graph_cache(sidecar_path: str, sha256: str, project_id: str, graph_id: str) -> None:
    """Add or update sha256 entry in the sidecar dict with project_id and graph_id."""
    data = {}
...
def strip_agents_section(seed_text):
    """Remove the '# Agents Population' section, keeping everything before and after it."""
    marker = "# Agents Population"
    if marker not in seed_text:
        return seed_text
    before, rest = seed_text.split(marker, 1)
    # Skip past the agents JSON block — find the next top-level section (#)
    next_section = rest.find("\n#")
    if next_section == -1:
        return before.rstrip()
    return (before.rstrip() + "\n\n" + rest[next_section:].lstrip()).strip()
...
def step3_prepare_simulation(base_url, project_id, graph_id, agent_count=20, session=None):
    ...
    result = api("post", base_url, "/api/simulation/prepare",
                 session=session, json={"simulation_id": simulation_id, "max_agents": agent_count})
    ...
    status_data = poll(check, interval=4, max_wait=600)
    elapsed = int(time.time() - t0)
