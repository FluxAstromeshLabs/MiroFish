...
def generate_report():
    ...
    then use `GET /api/report/generate/status` to query progress.
...
def chat_with_report_agent():
    ...
    The Report Agent can call retrieval tools during chat to answer questions.
...
def get_report_sections(report_id: str):
    """
    Get the list of generated sections (section-based output)
    
    The frontend can poll this API for generated section content without waiting for the full report.
...
def check_report_status(simulation_id: str):
    ...
    Used by the frontend to determine whether to unlock Interview.
...
def get_agent_log(report_id: str):
    ...
    Get each step of report generation in real time, including:
...
def stream_agent_log(report_id: str):
    """
    Get the complete Agent logs in one call
...
def get_console_log(report_id: str):
    ...
    Get console-style output during report generation in real time (INFO, WARNING, etc.).
    This differs from the structured JSON logs returned by `agent-log`,
    and is plain-text console-style logging.
...
def get_graph_statistics_tool():
    """
    Graph stats tool API (for debugging)
