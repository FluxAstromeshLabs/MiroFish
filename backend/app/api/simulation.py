...
Step 2: Zep entity reading/filtering and OASIS simulation preparation/execution (fully automated).
"""
...
INTERVIEW_PROMPT_PREFIX = "Using your persona, all prior memories, and actions, reply to me directly in plain text without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview prompts by adding a prefix to avoid Agent tool calls
...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes that match predefined entity types (labels are not limited to Entity).
...
def create_simulation():
    ...
    Note: parameters such as `max_rounds` are generated by the LLM and do not need to be set manually.
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, falls back to project if omitted
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # If `config_generated=True` and the files exist, preparation is treated as complete.
        ...
        # - running: running, which means preparation had already completed
        # - completed: completed, which means preparation had already completed
        # - stopped: stopped, which means preparation had already completed
        # - failed: failed during execution (but preparation was complete)
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is preparing but the files are complete, automatically update the status to ready.
            if status == "preparing":
                try:
            ...
            logger.warning(f"Simulation {simulation_id} check result: preparation not completed (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    try:
        ...
        # Check whether force_regenerate was requested
        ...
        # Load required info from the project
...
def get_prepare_status():
    ...
    1. Query progress of an ongoing task by task_id
    ...
    try:
        ...
        # No task_id provided; return error
        if not task_id:
            if simulation_id:
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Scan the reports directory for a report matching simulation_id,
    ...
    Returns:
        report_id, or None if not found
    """
    import json
    from datetime import datetime
    
    # reports directory: backend/uploads/reports
    # __file__ is app/api/simulation.py — go two levels up to reach backend/
    ...
    try:
        ...
        # Sort descending by creation time; return the most recent
...
def get_simulation_history():
    ...
    Used for the home-page history view; returns simulations enriched with project name, description, etc.
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If the market opens with...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Load simulation config (reads simulation_requirement from simulation_config.json)
            ...
            # Load run state (reads actual rounds set by the user from run_state.json)
            ...
            # Get the associated report_id (latest report for this simulation)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Suited for real-time viewing during generation
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suited for real-time viewing during generation
    - Returns extra metadata (file modification time, whether generation is in progress, etc.)
    - Returns partial info even if config generation is not yet complete
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: LLM reasoning for the generated config
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: max simulation rounds (caps long runs)
            "enable_graph_memory_update": false,   // optional: stream agent activity to Zep graph memory
            "force": false                         // optional: force restart (stops simulation and clears logs)
        }

    About the force parameter:
        - When enabled, stops any running simulation and clears run logs before restarting
        - Cleared files include: run_state.json, actions.jsonl, simulation.log, etc.
        - Does NOT clear config files (simulation_config.json) or profile files
        - Use when you need to re-run a simulation from scratch

    About enable_graph_memory_update:
        - When enabled, all agent activity is streamed to the Zep graph in real time
        - This lets the graph"remember"the simulation for later analysis or AI chat
        - Requires the simulation's project to have a valid graph_id
        - Uses a batch update mechanism to reduce API call volume
    ...
    try:
        ...
        # Check whether simulation is prepared
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is already running; call /stop first, or use force=true to force a restart"
                            }), 400

                # Force mode: clear run logs
                if force:
                ...
                # Process not found or finished; resetting status to ready
                logger.info(f"Simulation {simulation_id} Preparation complete; resetting to ready (previous status: {state.status.value}）")
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation not ready (status: {state.status.value}); call /prepare first"
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            graph_id = state.graph_id
            if not graph_id:
                # Try to get graph_id from project
                project = ProjectManager.get_project(state.project_id)
                if project:
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for real-time display in the frontend
    ...
    try:
        ...
        # Get actions for the current round (recent_actions shows the latest round only)
...
def get_simulation_timeline(simulation_id: str):
    ...
    Returns summary information per round
    """
    try:
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend display of agent activity rankings and action distribution
    """
    try:
...
def interview_agent():
    ...
    Note: requires the simulation environment to be running (post-loop, waiting for commands).
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // omit for dual-platform: interviews both platforms simultaneously
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden per item)
                                               // omit for dual-platform: each agent interviewed on both platforms
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What is your overall view on this?",  // required, question asked to all agents
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, fetch history for this agent only
