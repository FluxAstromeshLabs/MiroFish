...
Uses project-scoped context with server-side persisted state.
"""
...
def reset_project(project_id: str):
    """
    Reset the project state (used to rebuild the graph)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    ...
    try:
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "No documents were processed successfully. Check the file format."
            }), 400
        ...
        # Generate ontology
        logger.info("Calling the LLM to generate ontology definition...")
        ...
        # Save ontology into the project
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "The graph is already being built. Do not submit again. Use force: true to rebuild.",
                "task_id": project.graph_build_task_id
            }), 400
        
        # If forcing a rebuild, reset the state
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing to complete by checking each episode's processed state
                task_manager.update_task(
                    task_id,
