...
Zep node and edge list APIs use UUID cursor pagination.
This module wraps automatic pagination logic, including per-page retries,
and transparently returns the full result list to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Fetch a single page with exponential backoff. Retry only transient network or I/O errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, capped at max_items (default 2000), with retry per page."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination, with retry per page."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
