...
2. The simulation script polls that directory, executes commands, and writes responses into `responses/`.
3. Flask polls the response directory and reads the result.
"""
...
class SimulationIPCClient:
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Optional target platform.
                ...
                - None: In dual-platform simulations, interview both; in single-platform runs, interview that platform.
            timeout: Timeout.
            
        Returns:
            IPC response whose `result` field contains the interview output.
        """
        args = {
            "agent_id": agent_id,
            "prompt": prompt
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: Interview list, where each item is `{"agent_id": int, "prompt": str, "platform": str (optional)}`.
            platform: Default platform, overridden by each interview item's `platform`.
        ...
        Returns:
            IPC response whose `result` field contains all interview outputs.
        """
    ...
    def check_env_alive(self) -> bool:
        ...
        This is determined by reading `env_status.json`.
        """
...
class SimulationIPCServer:
    """
    Simulation IPC server used by the simulation scripts.
