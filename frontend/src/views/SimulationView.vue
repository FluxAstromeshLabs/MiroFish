...
const handleNextStep = (params = {}) => {
  ...
  // Pass the custom round count via query parameters when provided
  if (params.maxRounds) {
...
/**
 * Check for and close any running simulation.
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Detected a running simulation environment. Shutting it down...')
      ...
      try {
        ...
        // If graceful shutdown throws, try a force stop
        await forceStopSimulation()
      }
    } else {
      // The environment is not running, but the process may still exist, so check the simulation status
      const simRes = await getSimulation(currentSimulationId.value)
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Simulation status is running. Stopping it...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // Failure to check environment status should not block the remaining flow
