            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, the LLM analyzes the documents and generates an ontology structure suitable for simulation (entity types + relation types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Uses the generated ontology to chunk the documents and call the Zep API to build the knowledge graph and extract entities and relations.
                </div>
              </div>
...
// Current project ID, which may change from 'new' to a real ID
...
// Initialize by handling either a new project or an existing one
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files were found. Please return to Home and try again.'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling the in-progress build task
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Start graph data polling independently from task status polling
      startGraphPolling()
  ...
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Re-render only when the data changes
...
const startPollingTask = (taskId) => {
  ...
  // Then keep polling on an interval
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        if (projectResponse.success) {
          ...
          // Load the complete graph data at the end
...
const renderGraph = () => {
  ...
  // Draw edges, including a wide transparent clickable stroke
  ...
  // Transparent wide line used for clicking
  ...
  simulation.on('tick', () => {
    // Update every edge position, including the visible line and transparent click area
