            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content is ready, but the whole section may still be in progress) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show a special hint for the final answer -->
...
const toggleRawResult = (timestamp, event) => {
  // Store the button position relative to the viewport
  ...
  // After the DOM updates, adjust scroll so the button stays in the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract stats, including the "Related Forecast Facts: X" format
    const factMatch = text.match(/(?:Related Forecast Facts|\u76f8\u5173\u9884\u6d4b\u4e8b\u5b9e):\s*(\d+)/)
    ...
    // Extract core entities, including summary and related fact count
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract the reason for selecting interviewees
    const reasonMatch = text.match(/### (?:Interviewee Selection Reason|\u91c7\u8bbf\u5bf9\u8c61\u9009\u62e9\u7406\u7531)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcript|\u91c7\u8bbf\u5b9e\u5f55))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of the reason, excluding the ending summary paragraph
          currentReason.push(line.trim())
        }
      }
      
      // Save the final person's reason
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract the title, such as "Student" or "Education Professional"
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Handle the special case where the first question starts with "1."
      ...
      if (answerMatch) {
        ...
        const twitterMatch = answerText.match(/(?:\[Twitter Answer\]|【Twitter\u5e73\u53f0\u56de\u7b54】)\n?([\s\S]*?)(?=(?:\[Reddit Answer\]|【Reddit\u5e73\u53f0\u56de\u7b54】)|$)/)
        const redditMatch = answerText.match(/(?:\[Reddit Answer\]|【Reddit\u5e73\u53f0\u56de\u7b54】)\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic for older formats that only contain one platform marker
        if (!twitterMatch && redditMatch) {
          // Only a Reddit answer is present, so copy it only when it is not a placeholder
          ...
          // No platform markers were found, so treat the whole block as the answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes with support for multiple quote styles
    ...
    const summaryMatch = text.match(/### (?:Interview Summary and Key Takeaways|\u91c7\u8bbf\u6458\u8981\u4e0e\u6838\u5fc3\u89c2\u70b9)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts without limiting the count
    ...
    // Try to extract edge information when present
    ...
    // Try to extract node information when present
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain an independent platform selection state for each question-answer pair
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Get the currently selected platform for a question
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Set the selected platform for a question
    ...
    // Check whether this is a platform placeholder
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No reply from this platform)' || t === '[No reply]' || t === '(\u8be5\u5e73\u53f0\u672a\u83b7\u5f97\u56de\u590d)' || t === '\uff08\u8be5\u5e73\u53f0\u672a\u83b7\u5f97\u56de\u590d\uff09' || t === '[\u65e0\u56de\u590d]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fall back to the "number." format when there is no match
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbering is found, or only one match exists, return the whole answer
      if (matches.length <= 1) {
        const cleaned = answerText
          .replace(/^(?:Question\s*|[\u95ee\u9898])\d+[：:]\s*/, '')
          .replace(/^\d+\.\s+/, '')
          .trim()
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Splitting succeeded and the index is valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Splitting failed: return the full answer for the first question and empty strings for the rest
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has answers from both platforms, excluding placeholders
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switcher, shown only when both platforms have real answers
...
// Current active step for the top display
const activeStep = computed(() => {
  ...
  // If there is no active step, return the last completed one
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading second-level heading because the section title is already rendered outside
  ...
  // Handle line breaks: blank lines become paragraph breaks, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  
  // Wrap the result in paragraphs
  ...
  // Clean up cases where <p><br> is immediately followed by a block element
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> tags before paragraph starts that follow block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists so numbering stays incremental
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Handle scroll behavior in one place after the loop finishes
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Scroll to the top when completed, otherwise stay at the bottom with the latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to extract the content inside the <final_answer> tag
  ...
  // Try to find content after Final Answer: in multiple formats
  ...
  // If the content is long and contains markdown, try to remove the thought process first
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove thought-process content starting with Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, unchanged across states */
  font-weight: 500;
}
