    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a new simulation instance and load the template parameters for the simulated world.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using the graph and surrounding context, the system initializes simulated individuals and gives them distinct behavior and memory derived from the real-world seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM uses the simulation objective and real-world seeds to set time flow, recommendation logic, activity windows, posting frequency, event triggers, and other parameters.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Agents Active per Hour</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Uses the narrative direction to generate the initial activation events and trending topics that shape the world's starting state.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Round configuration: only shown after config generation -->
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, switch to custom mode and reduce the rounds so you can preview faster and lower the risk of runtime failures ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Experience-derived patterns and stylistic preferences</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Distinct Memory Imprint</span>
                <span class="dim-desc">Memories formed from the real-world seeds</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Social Relationship Network</span>
                <span class="dim-desc">Individual links and interaction topology</span>
              </div>
            </div>
...
// Log deduplication: keep track of the last key message
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generating Agent Personas' || newStage === '\u751f\u6210Agent\u4eba\u8bbe' || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === 'Generating Simulation Config' || newStage === '\u751f\u6210\u6a21\u62df\u914d\u7f6e' || newStage === 'generating_config') {
    phase.value = 2
    // Enter the config-generation phase and start polling config updates
    if (!configTimer) {
      addLog('Starting dual-platform simulation config generation...')
      startConfigPolling()
    }
  } else if (newStage === 'Preparing Simulation Scripts' || newStage === '\u51c6\u5907\u6a21\u62df\u811a\u672c' || newStage === 'copying_scripts') {
    phase.value = 2 // Still part of the config phase
  }
})

// Compute the auto-generated round count from the config without hardcoded defaults
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null before the config is generated
  }
  ...
  // Keep the max round count at or above 40 to avoid an invalid slider range
...
// Get the corresponding username from agent_id
...
// Compute the total number of related topics across all personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User-defined round count: pass max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation with custom rounds: ${customMaxRounds.value}`)
  } else {
    // User chose to keep the auto-generated round count, so do not pass max_rounds
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set the expected agent total using the prepare API response
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Emit detailed progress logs without duplicates
        ...
        // Emit message logs without duplicates
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit profile-generation progress logs only when the count changes
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Emit config-generation stage logs without duplicates
      ...
      if (data.config_generated && data.config) {
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup complete. Simulation can start')
...
const loadPreparedData = async () => {
  ...
  // Fetch the config using the real-time API
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // The config is not ready yet, so start polling
