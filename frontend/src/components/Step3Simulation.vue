          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Generic fallback for unknown action types or raw content -->
...
// Computed
// Show actions in chronological order, with the latest at the bottom
...
// Format elapsed simulation time based on the round count and minutes per round
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any previous polling loops
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so nothing leaks over from the previous run
  resetAllState()
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Setting maximum simulation rounds: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared old simulation logs and restarted the run')
      }
...
// Track the previous round for each platform so changes can be logged
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes for each platform and log them separately
      ...
      // Detect whether the simulation has completed via runner_status or per-platform completion
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: the backend may not have updated runner_status yet, but the platforms may already be done
      // Use twitter_completed and reddit_completed to detect that case
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform has completed, check whether all enabled platforms are complete
  // Use actions_count to determine whether a platform was enabled
  ...
  // Check whether every enabled platform is complete
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll so the user can inspect the timeline freely
