    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG short- and long-term memory is updating live' : 'Updating in real time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. Refresh the graph again shortly.</span>
...
const showSimulationFinishedHint = ref(false) // Hint shown after the simulation finishes
const wasSimulating = ref(false) // Track whether the graph was previously simulating

// Hide the simulation-finished hint
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Watch isSimulating and detect when the simulation ends
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating, so show the completion hint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle expanded/collapsed state for self-loop items
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // Reset the expanded state
}
...
const renderGraph = () => {
  ...
  // Process edge data and compute counts and indexes for edges between the same node pair
  const edgePairCount = {}
  ...
  // Track which edge index is currently being processed for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store detailed info for every self-loop edge
        }
      })
      return
    }
    ...
    // Compute curvature: spread multiple edges apart and keep a single edge straight
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly so parallel edges are clearly separated
      // Increase the curvature range as the edge count grows
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if the edge direction is opposite to the normalized direction
      // This keeps all edges in the same frame of reference and avoids overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust distance based on the number of edges between this node pair
      // Base distance is 150, then increase it for each extra edge
    ...
    // Add a pull toward the center so isolated node groups gather in the middle
  ...
  // Links - use path elements so curved edges are supported
  ...
  const getLinkPath = (d) => {
    ...
    // Compute the curve control point and adjust it dynamically based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the line direction using a distance ratio so the curve stays visible
    // Increase the offset ratio as more edges are present
  ...
  // Compute the curve midpoint for label positioning
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier formula B(t) = (1-t)^2 P0 + 2(1-t)t P1 + t^2 P2, with t=0.5
  ...
  // Link label background (white improves text readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position without restarting the simulation so clicks and drags are distinct
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect when a real drag begins by checking the movement threshold
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only after the first confirmed drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let the simulation cool down if a real drag occurred
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    // Update the curved paths
    link.attr('d', d => getLinkPath(d))
    
    // Update edge label positions without rotation so they stay easier to read
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Remove rotation and keep the label horizontal
    })
  ...
  // Click empty space to close the detail panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
/* Hint styles shown after the simulation finishes */
