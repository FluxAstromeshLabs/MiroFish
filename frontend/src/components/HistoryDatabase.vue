      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show overflow hint if more files exist -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be started live and cannot be replayed from history.</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Target state waiting to be applied

// Card layout configuration with a wider aspect ratio
...
const containerStyle = computed(() => {
  ...
  // Expanded state: compute height dynamically from the card count
  ...
  // Compute the required height: rows * card height + (rows - 1) * gap + a small bottom gap
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute the card count for the current row so each row stays centered
    ...
    // Expand downward and add spacing from the title
    ...
    // Collapsed state: fan-style stack
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'
...
// Format the date, showing only the date portion
...
// Generate a title from the simulation requirement using the first 20 characters
...
// Format simulation_id for display using the first 6 characters
...
// Format round display as current round / total rounds
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state even if an animation is already running
        pendingState = shouldExpand
        
        // Clear the previous debounce timer because the latest scroll intent wins
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If an animation is running, only record the state and handle it afterward
        if (isAnimating) return
        
        // Skip processing when the target state matches the current state
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state transitions to avoid rapid flicker
        // Use a shorter delay for expansion (50ms) and a longer one for collapse (200ms)
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending state still needs to run
          if (pendingState === null || pendingState === isExpanded.value) return
          
          // Enable the animation lock
          isAnimating = true
          isExpanded.value = pendingState
          pendingState = null
          
          // Release the lock when the animation ends, then check for a pending state change
          setTimeout(() => {
            isAnimating = false
            
            // After the animation ends, check for a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Wait briefly before applying it to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Shrink the viewport bottom via rootMargin so expansion triggers only after more scrolling
...
onMounted(async () => {
  // Wait for the DOM to finish rendering before loading data
  await nextTick()
  await loadHistory()
  
  // Initialize the observer after the DOM is rendered
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Anchor from the top-left so height changes only expand downward */
...
.cards-container {
  ...
  /* min-height is computed by JS and adapts to the number of cards */
}
...
/* Low-saturation palette inspired by Morandi colors */
